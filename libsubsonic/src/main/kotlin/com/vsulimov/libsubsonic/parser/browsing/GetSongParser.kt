...
internal object GetSongParser {
    ...
    fun parseSong(json: JSONObject): Child {
        ...
        val replayGain = json.optJSONObject("replayGain")?.let {
            ReplayGain(
                albumGain = if (it.has("albumGain")) it.optDouble("albumGain") else null,
                artistGain = if (it.has("artistGain")) it.optDouble("artistGain") else null,
                trackPeak = if (it.has("trackPeak")) it.optDouble("trackPeak") else null,
                albumPeak = if (it.has("albumPeak")) it.optDouble("albumPeak") else null,
                trackGain = if (it.has("trackGain")) it.optDouble("trackGain") else null
            )
        }
