...
import com.vsulimov.libsubsonic.parser.system.SubsonicPingResponseParser
...
class SubsonicClient(baseUrl: String, clientName: String = DEFAULT_CLIENT_NAME) {
     ...
     *
     * @return [SubsonicResult.Success] containing server details ([SubsonicPingResponse]).
     * @return [SubsonicResult.Failure] if the server is unreachable or credentials are invalid.
     */
    suspend fun ping(): SubsonicResult<SubsonicPingResponse> = executor.execute("ping.view") { jsonObject ->
        SubsonicPingResponseParser.parse(jsonObject)
    }
