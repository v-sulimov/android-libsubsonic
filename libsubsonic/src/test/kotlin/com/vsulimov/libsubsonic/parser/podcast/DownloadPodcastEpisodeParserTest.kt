...
class DownloadPodcastEpisodeParserTest {
    ...
    fun `parse extracts status and version from successful response`() {
        ...
        val response = DownloadPodcastEpisodeParser.parse(JSONObject(jsonString))
    ...
    fun `parse captures server metadata fields`() {
        val response = DownloadPodcastEpisodeParser.parse(
