...
class SubsonicResponseParserTest {
    ...
    fun `parse failure state returns mapped SubsonicError`() {
        ...
        assertEquals(expected = SubsonicErrorCode.WRONG_CREDENTIALS, actual = result.error.code)
    ...
    fun `parse blank string returns failure with descriptive message`() {
        ...
        assertTrue(result is SubsonicResult.Failure)
        assertEquals(expected = SubsonicErrorCode.GENERIC_ERROR, actual = result.error.code)
    ...
    @Test
    fun `parse failure with error block missing message returns unknown api error`() {
        val json = """
            {
                "subsonic-response": {
                    "status": "failed",
        ...
        assertTrue(result is SubsonicResult.Failure)
        assertEquals(expected = SubsonicErrorCode.NOT_AUTHORIZED, actual = result.error.code)
        assertEquals(expected = "Unknown API error", actual = result.error.message)
    }
